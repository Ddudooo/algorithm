...
안테나가 커버할 수 있는 범위를 계산하여

커버할 수 없는 지역 범위를 몇개의 안테나가 필요한지

안테나 별로 순회하면서 처리

입력으로 주어지는 안테나 위치가 오름 차순으로 주어짐으로

안테나 입력을 순회하면서

이전 안테나 위치간 거리를 계산할 수 있다.

이 거리를 통해 커버할 수 없는 지역을 계산할 수 있으므로 해당 계산을 통해

결과를 도출 한다.

처음 시도 당시 `N`을 무식하게 다 순회하려고 하였으나

입력값이 최대 200,000,000 까지 되므로... 미커버지역에 대한 계산으로 바꿔서 처리하였다.