...
다익스트라 알고리즘을 통해 계산하여 처리할려 했으나

효율성에서 통과가 안되서, 익숙한 플로이드 와샬 알고리즘으로 처리하였다.

결과 값으로는 `i` 번째 지점을 경유지를 했을 때 값이 가장 작은 경우인데

`i` 가 `s`, 즉 시작점일때는 같이 타는 지역없이 바로 직행할 때이며,

이 외에는 해당 경유지 `i`까지 같이 타고 이후에 나눠서 갈때 값으로 보면 된다.

다익스트라 알고리즘도 풀려야 할텐데, 안풀리는 걸 보니

다익스트라 알고리즘을 다시 한번봐야겠다.