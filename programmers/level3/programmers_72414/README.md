...
동영상의 재생시간을 1초 단위로 배열로 잡은뒤

시청 로그를 받아 시작 시간부터 종료시간까지 재생시간 배열에 +1 해주어

해당 구간에 시청중이였음을 확인할 수 있게 하였다.

이후 광고 구간을 처음 시작 `00:00:00` 부터 광고 종료시간까지의

시청자수를 모두 더하여, 해당 구간의 누적합을 구하여

해당 누적합이 최대일때 시작 시간을 구하였다...

처음에 해매서 찾아봤는데, 검색 결과 대부분이 그냥 복붙만 해둔 상태라

이해가 안되서 그냥 다지우고 풀어서 해결...