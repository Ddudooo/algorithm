...
노드간 추천인 정보와 수익율 분배에 관해 정리되면 완료

노드가 아무리 많아야 10000 개라

`enroll`, `referral`, `seller`, `amount`를 부담없이 순차 순회하여 노드간 정보를 업데이트 시켜주면 완료 된다.

이름으로 노드정보를 주기에, 조회 편의상 `Map`을 사용하여 노드 정보를 다시 받아왔다.

판매 정보 순회시에 상위 노드 업데이트는 노드 메소드를 재정의 하여 상위 노드 재귀호출로 처리하였다.
