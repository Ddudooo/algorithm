...
1번째 부터 n번째 까지 스티커를 조건에 맞춰 최대 합을 구해야 하는 문제

한 스티커를 골랐을 경우 그 인근 스티커는 고를 수 없다

단순하게 1 부터 N까지 순회하면서

이전 스티커를 골랐을 경우와 안골랐을 경우에 대한 합산값을 비교해 가면서

해당 스티커까지 주어졌을때 합을 연산해준다

다만, 원형 구조로 첫번째 스티커를 골랐을 경우 마지막 스티커를 고르지 못하므로
